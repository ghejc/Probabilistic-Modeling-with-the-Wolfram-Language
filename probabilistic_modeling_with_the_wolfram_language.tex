...
\includegraphics[scale=0.8]{images/normal_distribution.pdf}
\end{example}
...
\end{mathematica}
 ...
 \includegraphics[scale=0.9]{images/gaussian_mixture_model.pdf}   
...
We iterate the index $j$ from $N$ down to $0$ and stop as as soon as the condition $\Phi(j) < r$ is fulfilled. Finally we increment the index $j\rightarrow j+1$. This procedure generates an index sequence with the desired distribution.\\
...
The histogram shows the counts of the data samples generated from an empirical distribution for each category. The counts are indeed numerically close to the given weights.\\
 ...
 \includegraphics[scale=1]{images/sampling_from_discrete_distribution.pdf}   
\end{example}
...
The Wolfram Language provides the function \texttt{HistogramDistribution}, which can be constructed from given data and can be also used to generate new data samples using \texttt{RandomVariate}. In the code example we don't use it, but implement the sampling procedure as described above. \\

The function \texttt{sampleFromHistogram} has three arguments: the bins $b_{1:N+1}$, the normalized bin heights $h_{1:N}$ and the number of samples $n$. It calculates first the center coordinates and widths of the bins and then the cumulative sum of the bin heights, where zero is appended at the beginning of the list. $n$ random numbers are generated to sample a bin index $j$. This is done by subtracting the random number from each element of the cumulative sum and keep only the largest element of the negative results. The index is then used to generate a uniformly distributed number inside the corresponding bin.
\begin{mathematica}
sampleFromHistogram[bins_, binHeights_, n_Integer] := Module[
  {widths, centers, cumSum, rk, j, x},
  widths = Differences[bins];
  centers = MovingAverage[bins, 2];
  cumSum = Prepend[Accumulate[binHeights], 0]; 
  rk = RandomReal[1, n]; 
  j = Flatten[Map[Ordering[Select[cumSum - #, NonPositive], -1] &, rk]];
  x = Map[RandomReal[{centers[[#]] - widths[[#]]/2,
                      centers[[#]] + widths[[#]]/2}] &, j];
  Return[x]]
...
\end{mathematica}
 \includegraphics[scale=0.9]{images/histogram_sampling.pdf}   
...
\end{mathematica}
 ...
 \includegraphics[scale=1]{images/normal_distributions_from_data.pdf}
...
\end{mathematica}
 ...
 \includegraphics[scale=0.9]{images/rejection_sampling.pdf}

\end{example} 

Finding a pdf, which is easy to sample from and has a similar functional form as the distribution of interest, can be achieved with the help of a Gaussian Mixture Model $p_{GMM}(x\,\vert\,\boldsymbol{\alpha},\boldsymbol{\mu},\boldsymbol{\sigma^2})$.
