...
\item
Etienne Bernard's book \textbf{Introduction in Machine Learning} \cite{bernard2023} introduces neural networks and machine learning from the perspective of the Wolfram language.   
\item
Christopher Bishop's book \textbf{Deep Learning} \cite{bishop2024} is a brilliant book, which covers the whole field of deep learning in a comprehensive self-contained manner and is therefore useful both for newcomers as well as for experienced readers. Especially the first two chapters about probabilities and standard distributions are a valuable addition to the material presented in this book. 
\end{itemize}
...
A probability density of two or more random variables is called a joint probability density e.g. $p(x,y)$ in the case of two random variables $x$ and $y$. The normalization condition is given by 
  \begin{equation}
  	\index{joint probability density}
...
dist = ProbabilityDistribution[Sqrt[2]/(Pi*\[Alpha])/(1 + (x/\[Alpha])^4),
                               ...
                               Assumptions -> {\[Alpha] > 0}];
\end{mathematica} 
The \texttt{Assumptions} option $\alpha > 0$ is necessary to guarantee the non-negativity of the pdf. The function, which is provided as the first argument, must be normalized to unity or if not, the option \texttt{ Method -> "Normalize"} must be explicitly set. An equivalent definition would be
\begin{mathematica}
...
This decomposition is not unique. Any permutations of $\mathbf{x}$ is possible here.
  ...
  \label{eq:decomposition2}
    p(x_{1:n}\,\vert\, \mathbf{y},\boldsymbol{\theta})=p(x_1\,\vert\, \mathbf{y},\boldsymbol{\theta})p(x_2\,\vert\, x_1,\mathbf{y},\boldsymbol{\theta})p(x_3\,\vert\, x_1,x_2,\mathbf{y},\boldsymbol{\theta})\ldots p(x_{n}\,\vert\, x_{1:n-1},\mathbf{y},\boldsymbol{\theta})
  \end{equation}

\begin{remark}
Both sides of eq. \ref{eq:decomposition2} are equivalent in terms of the number of states and therefore contain the same amount of information. Let's assume that each random variable $x_{i}$ with $i=1,2\ldots n$ can take $q$ values. Then the number of states of the rhs is $q^n - 1$, where the $-1$ comes from the normalization condition. Each univariate conditional probability density conrtibutes $q-1$ from the variable before the condition sign and $q^m$ from the $m$ variables after the condition sign. Therefore the number of states on the lhs is of the form of a geometric series $(q-1)\left(1+q+q^2+\ldots + q^{n-1}\right)$ with the sum $(q-1)\frac{q^n-1}{q-1}$, which is identical to the result of the rhs.
\end{remark}

\begin{exercise}
	Show that in the general case where each random variable  $x_{i}$ can take values  $q_{i}$ the number of states of the rhs is $q_1 q_2\ldots q_n - 1$ and the lhs is $q_1 - 1 + (q_2 - 1)q_1 + (q_3 - 1)q_1 q_2 + \ldots (q_n - 1)q_1 q_2 \ldots q_{n-1}$ and that both sides are equal. 
\end{exercise}

\textit{The question is: what is the best choice for this decomposition?}\\

There is no general answer here, but the strategy would be to exploit the conditional independence of random variables as often as possible by applying
  \begin{equation}
    p(x\,\vert\, y,\theta)=p(x\,\vert\, \theta)
  \end{equation}
 The statement $x$ is conditionally independent from $y$ means that the functional form of the probability density of $x$ is the same regardless of the value of $y$. This requires additional or prior knowledge, which may or may not be available.  

For example, using the Markov property that $x_i$ only depends on $x_{i-1}$ in eq. \ref{eq:decomposition2} simplifies the expression significantly to 

  \begin{equation}
	p(x_{1:n}\,\vert\, \mathbf{y},\boldsymbol{\theta})=p(x_1\,\vert\, \mathbf{y},\boldsymbol{\theta})p(x_2\,\vert\, x_1,\mathbf{y},\boldsymbol{\theta})p(x_3\,\vert\, x_2,\mathbf{y},\boldsymbol{\theta})\ldots p(x_{n}\,\vert\, x_{n-1},\mathbf{y},\boldsymbol{\theta}) 
  \end{equation}

The number of states is reduced to $(q-1)\left(1 + (n-1)q\right)$ and is therefore only quadratically in $q$ compared to $q^n$.
\\

\begin{remark}
Expressions with a product of probability densities will occur very often and are subject to numerical underflow. It is therefore recommended to perform numerical evaluations always with the logarithm of a probability density. It does not matter if it is the natural logarithm (we use $\ln$ or $\log$ interchangeably) or the logarithm with base 2 or 10 (in this case we write explicitely $\log_2$ or $\log_{10})$. The crucial point is that a product of pdf's is converted into a sum of logarithms of pdf's, which is numerically more stable.
\end{remark}
...
is a valid operation. The next example highlights some of the issues you have to deal with. 
\end{remark}

\begin{example}[Entropy]
\index{entropy}
The entropy for a continuous random variable $x\sim p(x)$ (or differential entropy) is defined as
  \begin{equation}
  \label{eq:differential_entropy}
  \index{differential entropy}
    h(x)=-\int_{-\infty}^{+\infty}p\left(x\right)\log_{2}\left(p(x)\right)dx
  \end{equation}
From a physical perspective this definition has a flaw: a probability density $p(x)$ has a physical unit if $x$ has a physical unit. Taking the logarithm of non-dimensionless quantity is a forbidden operation. To avoid the problem, an arbitrary factor $\Lambda$ with the same unit as $p(x)$ has to be added.

  \begin{equation}
    h(x)=-\int_{-\infty}^{+\infty}p\left(x\right)\log_{2}\left(\frac{p(x)}{\Lambda}\right)dx=
    -\int_{-\infty}^{+\infty}p\left(x\right)\log_{2}\left(p(x)\right)dx + \log_{2}\left(\Lambda\right)
  \end{equation}
...
If we insert eq. \ref{eq:discreteprobabilitydensity} in this expression, we get 
  \begin{equation}
   \begin{aligned}
    h(x)&=-\sum_{i=-\infty}^{+\infty}\int_{-\infty}^{+\infty}P_i\delta\left(x-x_i\right)\log_{2}\left(p(x)\right)dx
    =-\sum_{i=-\infty}^{+\infty}P_i\log_{2}\left(p(x_{i})\right)\\
    &=-\sum_{i=-\infty}^{+\infty}P_i\log_{2}\left(P_i\delta(0)\right)
    =-\sum_{i=-\infty}^{+\infty}P_i\log_{2}\left(P_i\right) - \log_{2}\left(\delta(0)\right)
   \end{aligned}
  \end{equation}
The step, where we replaced $p(x_{i})$ by $P_i\delta(0)$ treated $\delta(x)$ as a function, which is zero for any non-zero argument, therefore the sum inside the logarithm is reduced to only one term.\\

The last term $\log_{2}\left(\delta(0)\right)$ is divergent, but independent of the random variable $x$ and therefore plays no role, because entropy is anyhow defined up to an additive factor due to the arguments presented above.\\
...
Based on the definition of entropy from eq. \ref{eq:differential_entropy}, we will derive a continuous probability distribution, which is very important in physics: the Boltzmann distribution. It is the result of minimizing the entropy with respect to the probability density $p(x)$ by including the constraints $\int_{-\infty}^{+\infty} p(x)dx=1$ and $\int_{-\infty}^{+\infty}xp(x)dx=\mu$ via Lagrange multiplicators. 
\begin{equation}
h(x)=-\int_{-\infty}^{+\infty}p\left(x\right)\log\left(p(x)\right)dx + \alpha \left(\int_{-\infty}^{+\infty}p(x)dx - 1\right)dx + \beta \left(\int_{-\infty}^{+\infty}xp(x)dx - \mu\right)
\end{equation}
We bring every term under the integral.
\begin{equation}
h(x)=\int_{-\infty}^{+\infty}dx\left[-p\left(x\right)\log\left(p(x)\right) + \alpha \left( p(x) - \delta(x)\right) + \beta \left(xp(x) - \mu\delta(x)\right)\right]
\end{equation}
The minimum can be found by taking the derivative with respect to $p(x)$ of the expression under the integral, ignoring that $p(x)$ is a function of $x$ and not a variable and setting the integrand to zero.
\begin{equation}
	-\log\left(p(x)\right) - 1 + \alpha + \beta x = 0
\end{equation}
The solution is
\begin{equation}
    \label{eq:boltzmann_distribution}
	p(x) = \exp\left(1 - \alpha - \beta x \right)
\end{equation}
The domain of $p(x)$ cannot be $-\infty,+\infty$, because the integrals would diverge, but any finite negative number as lower limit would work. In physics, $x$ would be the energy and $\beta$ the inverse temperature. If the lowest energy level would be $-\infty$, the physical system would be unstable.\\
 
$\alpha$ and $\beta$ can be determined by using eq. \ref{eq:boltzmann_distribution} to solve the two constraints.
\end{example}

\begin{exercise}
	Repeat the last example with three constraints
	\begin{equation}
		  \begin{aligned}
h(x)&=-\int_{-\infty}^{+\infty}p\left(x\right)\log\left(p(x)\right)dx + \lambda_1 \left(\int_{-\infty}^{+\infty}p(x)dx - 1\right)dx\\
& + \lambda_2 \left(\int_{-\infty}^{+\infty}xp(x)dx - \mu\right) + \lambda_3 \left(\int_{-\infty}^{+\infty}(x-\mu)^2 p(x)dx - \sigma^2\right)
  	\end{aligned}
	\end{equation}
	and show that the solution is the Normal distribution.
\end{exercise}

The next paragraphs will discuss important examples of discrete probability densities, which are encountered in many different contexts.
...
A custom discrete probability function from a list of values $x_i$ with their associated probabilities $P_i$
can be defined with \texttt{EmpiricalDistribution}.
\begin{mathematica}
values = {0, 1, 2, 3};
probabilities = {0.2, 0.3, 0.3, 0.2};
p = EmpiricalDistribution[probabilities -> values];
\end{mathematica}
\texttt{PDF[p,x]} returns \\

\texttt{0.2 Boole[0 == x] + 0.3 Boole[1 == x] + 0.3 Boole[2 == x] + 
 0.2 Boole[3 == x]}\\

where the \texttt{Boole} function is 1 if the argument is \texttt{True}, otherwise 0.
...
\begin{example}[The Central Limit Theorem]
\index{central limit theorem}
This theorem states that the mean $\mu$ of an independent random sample $x_1$,$x_2$,...$x_N$ with $x_i$ being distributed according to an arbitrary distribution $p(x)$ tends to a normal distribution in the limit $N\rightarrow\infty$. We choose for $p(x)$ the uniform distribution $\mathcal{U}\left(x\,\vert\,-1,+1\right)$, which has zero mean and $\sigma^2=\frac{1}{3}$ (see next exercise).
	
Starting point as usual is the joint probability density $p(\mu,x_1,x_2,\ldots,x_N)$, which we marginalize over $x_1$,$x_2$,...$x_N$ to get $p(\mu)$.
\begin{equation}
	p(\mu) = \int_{-\infty}^{+\infty}dx_1 \int_{-\infty}^{+\infty}dx_2\ldots \int_{-\infty}^{+\infty}dx_N p(\mu,x_1,x_2,\ldots,x_N)
\end{equation}
The expression can be further simplified
\begin{equation}
	p(\mu) = \int_{-\infty}^{+\infty}dx_1 \int_{-\infty}^{+\infty}dx_2\ldots \int_{-\infty}^{+\infty}dx_N p(\mu \,\vert\, x_1,x_2,\ldots,x_N)\prod_{i=1}^{N}p(x_i)
\end{equation}
Because $\mu$ is the sample mean, the conditional pdf $p(\mu \,\vert\, x_1,x_2,\ldots,x_N)$ can be replaced by $\delta\left(\mu - \frac{1}{N}(x_1+x_2+\ldots+x_N)\right)$. We also use the following integral representation of the delta function.
\begin{equation}
	\delta(x) = \frac{1}{2\pi}\int_{-\infty}^{+\infty}dk e^{ikx}
\end{equation}
Then the expression for $p(\mu)$ can be rewritten as
\begin{equation}
	p(\mu) = \frac{1}{2\pi}\int_{-\infty}^{+\infty}dk e^{ik\mu}\prod_{i=1}^{N} \int_{-\infty}^{+\infty}dx_i p(x_i) e^{-ik x_i/N}
\end{equation}
Now we use the fact that $p(x_i)$ is a uniform distribution in the interval $(-1,+1)$.
\begin{equation}
	p(\mu) = \frac{1}{2\pi}\int_{-\infty}^{+\infty}dk e^{ik\mu}\prod_{i=1}^{N} \frac{1}{2}\int_{-1}^{+1}dx_i e^{-ik x_i/N}
\end{equation}
The integration over the $N$ variables $x_i$ can now be carried out.
  \begin{equation}
	\begin{aligned}
	p(\mu)& = \frac{1}{2\pi}\int_{-\infty}^{+\infty}dk e^{ik\mu} \left(\frac{-N}{2ik}\right)^N\left(e^{-ik/N} - e^{ik/N}\right)^N \\
	&=\frac{1}{2\pi}\int_{-\infty}^{+\infty}dk e^{ik\mu} \left(\frac{N}{k}\sin(k/N)\right)^N
	\end{aligned}
\end{equation}
For large $N$ the sine function can be expanded as $\sin(x)=x-\frac{x^3}{3!}+\ldots$ ignoring higher-order terms.
  \begin{equation}
	\begin{aligned}
		p(\mu)& \approx \frac{1}{2\pi}\int_{-\infty}^{+\infty}dk e^{ik\mu} \left(1-\frac{(k/N)^2}{3!}\right)^N \\
		&\approx \frac{1}{2\pi}\int_{-\infty}^{+\infty}dk e^{ik\mu} e^{-k^2/(3!N)}\\
		&= \frac{1}{\sqrt{2\pi}}\mathcal{F}\{e^{-k^2/(3!N)}\}[\mu]
	\end{aligned}
\end{equation}
using $\left(1+\frac{x}{N}\right)^N = e^x$ for $N\rightarrow\infty$ and the Fourier transform $\mathcal{F}$ in the last step.

Using the Wolfram Language to solve the integral
\begin{mathematica}
1/Sqrt[2*Pi]*FourierTransform[Exp[-k**2/(Factorial[3]*N)], k, \[Mu]]
\end{mathematica}
gives
\begin{equation}
	p(\mu) \approx \sqrt{\frac{3N}{2\pi}}e^{-3N\mu^2/2}
\end{equation}
which is a normal distribution with $\mu'=0$ and $\sigma'^2=\frac{\sigma^2}{N}=\frac{1}{3N}$.\\

The following code generates $10^6$ uniformly distributed values, reshapes the array to a matrix and takes the mean over each column vector.
The distribution of the mean values is compared to the expected Normal distribution.
\begin{mathematica}
NumPoints = 1000000;
x = RandomVariate[UniformDistribution[{-1, 1}], NumPoints];
x = ArrayReshape[x, {1000, 1000}];
Show[Histogram[Mean[x], 20, "PDF"], 
Plot[PDF[NormalDistribution[0, 1/Sqrt[3000]], u], {u, -0.05, 0.05}]]
\end{mathematica}


 \includegraphics[scale=1, center]{images/central_limit_theorem.pdf}

The mean value distribution has the expected Gaussian shape.
...
For some applications a smooth version of a histogram is preferred, which can be constructed by assigning a Gaussian distribution $\mathcal{N}\left(x_i,\sigma^2\right)$ to each data point $x_i$ and summing up the area under the pdf in each bin from each contribution to yield the count value of the corresponding histogram. The result depends very much on the choice of $\sigma$. The histogram is exactly reproduced in the limit $\sigma\rightarrow\infty$.

\begin{definition}[Binned Kernel Density Estimator]
  \label{th:kde}
  \index{binned kernel density estimator}
  A binned KDE (Kernel Density Estimator) is defined by
  \begin{equation}
   \label{eq:binned_kde}
    p_{bKDE}(b\,\vert\,\sigma^2,b_{1:M}) = \begin{cases} \sum_{i=1}^N \int_{b_j}^{b_{j+1}}dx \mathcal{N}\left(x\,\vert\,x_i,\sigma^2\right) & b\in [b_j, b_{j+1}] \\
0 & otherwise \\
\end{cases}
  \end{equation}
  where $b_j$ is an ordered sequence of $M$ bin values.
\end{definition}

A Wolfram Language implementation \texttt{BinnedKernelDensityEstimate} uses the fact that the integral over the normal distribution can be expressed as the difference of two error functions.
  \begin{equation}
    \int_{b_j}^{b_{j+1}}dx \mathcal{N}\left(x\,\vert\,x_i,\sigma^2\right) = \frac{1}{2}\left[\texttt{erf} \left(\frac{b_{j+1}-x_i}{\sqrt{2}\sigma}\right)-\texttt{erf} \left(\frac{b_j-x_i}{\sqrt{2}\sigma}\right)\right]
  \end{equation}
  
The function returns a normalized count for each bin.
  
\begin{mathematica}
BinnedKernelDensityEstimate[data_List, bins_List, bandwidth_Real] := Module[
   {d, w, counts},
   w = Sqrt[2] * bandwidth;
   counts = Sum[MapThread[
                Erf[(#2 - d)/w] - Erf[(#1 - d)/w] &, {bins[[;; -2]], bins[[2 ;;]]}],
                {d, data}];
   Return[counts / Apply[Plus, counts]]];
   
data = RandomVariate[NormalDistribution[], 100];
h = HistogramList[data];
counts1 = BinnedKernelDensityEstimate[data, h[[1]], 0.5];
counts2 = h[[2]] / Apply[Plus, h[[2]]] // N;
DiscretePlot[{counts1[[i]], counts2[[i]]}, {i, 1, Length[counts1]}, 
             ExtentSize -> Full]
\end{mathematica}

A comparison with the function \texttt{HistogramList} shows a good agreement for the chosen bandwidth of 0.5.
  
 \includegraphics[scale=0.9, center]{images/binned_kernel_density_estimate.pdf}

The integration over each bin in eq. \ref{eq:binned_kde} is useful to get directly a probability value, but it is not necessary. A meaningful kernel density estimation can be done using a Gaussian pdf as kernel and therefore yields a function instead of sequence of values.

\begin{equation}
\label{eq:gaussian_kde}
\index{gaussian kde}
    p_{KDE}(x\,\vert\,\sigma^2) = \frac{1}{N}\sum_{i=1}^N \mathcal{N}\left(x\,\vert\,x_i,\sigma^2\right)
\end{equation}

The Wolfram Language provides such a function under the name \texttt{SmoothKernelDistribution[data,bw,ker]} with the kernel argument \texttt{ker} set to \texttt{"Gaussian"}, which returns a probability distribution.

\begin{mathematica}
data = RandomVariate[NormalDistribution[], 100];
dist = SmoothKernelDistribution[data, 0.5, "Gaussian"];
h1 = HistogramList[RandomVariate[dist, 100]];
(* Apply the binning of histogram 1 to histogram 2 *)
h2 = HistogramList[data, {h1[[1]]}];
counts1 = h1[[2]] / Apply[Plus, h1[[2]]] // N;
counts2 = h2[[2]] / Apply[Plus, h2[[2]]] // N;
DiscretePlot[{counts1[[i]], counts2[[i]]}, {i, 1, Length[counts1]},
             ExtentSize -> Full]
\end{mathematica}
 ...
 \includegraphics[scale=0.9, center]{images/kernel_density_estimate.pdf}
...
An extension of Gaussian Mixture Models for conditional distribution estimation are given by Mixture Density Networks.

\begin{definition}[Mixture Density Network]
	\label{th:mixturedensitynetworks}
	\index{mixture density networks}
	A Mixture Density Network is defined by
	\begin{equation}
		p_{MDN}(\mathbf{x}\,\vert\,\boldsymbol{z}) = \sum_{i=1}^N \alpha_i(\boldsymbol{z}) \mathcal{N}\left(\mathbf{x}\,\vert\,\mu_i(\boldsymbol{z}),\sigma_i(\boldsymbol{z})\right)
	\end{equation}
	with $\sum_{i=1}^N \alpha_i(\boldsymbol{z})=1$.
\end{definition}
The parameters $\alpha_{1:N}$, $\mu_{1:N}$ and $\sigma_{1:N}$ are functions of $\boldsymbol{z}$ and are approximated by neural networks.

For demonstrating the capabilities of this method, we take the following two-dimensional data. The density of points is the probability measure. 

 \includegraphics[scale=0.9, center]{images/mixture_density_network_data.pdf}
 
The corresponding histogram gives a rough approximation of the distribution described by these data.

 \includegraphics[scale=0.9, center]{images/mixture_density_network_histogram.pdf}

\section{Normalizing Flows}
\index{normalizing flows}
Normalizing flows are another way to construct a probability distribution from its samples. A Normalizing Flow is a mapping of a simple probability distribution e.g. a Normal distribution into a more complex distribution by a sequence of invertible transformations $f_i$. The complete transformation $f$ is given by $f_1 \circ f_2 \circ \ldots f_{N-1} \circ f_N$ and is also invertible, which means that $f^{-1}$ exists. For such a function to be useful, it must also be necessary to differentiate $f$ with respect ot its arguments.
...
The SemiCircle pdf describes the distribution of eigenvalues of an infinite symmetric random matrix with elements distributed according to a Gaussian probability density.

\begin{mathematica}
mat = ArrayReshape[RandomVariate[NormalDistribution[], 1000000], {1000, 1000}];
eigenvals = Eigenvalues[Symmetrize[mat]];
Show[Histogram[eigenvals/Max[Abs[eigenvals]], 20, "PDF"]]
\end{mathematica} 

 \includegraphics[scale=1.0, center]{images/semi_circle_distribution_from_eigenvalues.pdf}

The corresponding function in the Wolfram Language is \texttt{WignerSemicircleDistribution[a,r]}, which describes a semi circle of radius \texttt{r} centered at \texttt{a}.
\end{example} 
...
\begin{example}[A moving ball in a two-dimensional non-transparent box]
...
\begin{mathematica}
ParticleFilterInit[numberOfParticles_, velocity_, boundaries_] := Module[
    {pos, v, vel, particles},
...
\begin{example}[One-dimensional Random Walk]
\label{ex:randomwalk}
\index{random walk}
...
\end{equation}
Using eq. \ref{eq:randomwalk} $p\left(x_{n+1}\right)$ can be determined iteratively starting with the initial distribution $p\left(x_{0}\right)=\delta\left(x_{0}\right)$.
...
If we assume that $n=2m$ is an even number, the coefficient of the term $\delta\left(x_{n}\right)$ tells us the probability to find the particle at its starting position after $n$ steps. In the limit $m\rightarrow\infty$, the probability goes to zero.\\

We can check this with the following code, where we set $\alpha$ to $\frac{1}{2}$.
...
\includegraphics[scale=0.8,center]{images/displacement_after_n_steps.pdf}

For large $n$ the displacement can be described by a Normal distribution $\mathcal{N}\left(0,\sigma^2\right)$,
where $\sigma^2$ is proportional to the number of steps $n$.\\

In the Wolfram Language a random walk sequence generator can be implemented with the following function
\begin{mathematica}
RandomWalkSequence[alpha_, n_]:= NestList[# + RandomChoice[{alpha, 1 - alpha}->
                                          {1, -1}] &, 0, n];
\end{mathematica}
The function \texttt{RandomChoice} selects with probability \texttt{alpha} the value $+1$ and with probability \texttt{1-alpha} the value $-1$ and adds it to the last evaluation, which is performed iteratively \texttt{n} times using \texttt{NestList} starting with $0$.\\
Wolfram Language has already a built-in functions \texttt{RandomWalkProcess} and \texttt{RandomFunction} for that purpose.
...
\includegraphics[scale=0.8,center]{images/random_walk_process.pdf}

\end{example}

\begin{example}[White Noise Process]
Another important random process is White Noise. The sequence consists simply of samples from a Normal distribution. 
\begin{equation}
x_{n} \sim \mathcal{N}\left(0,\sigma^2\right)
\end{equation}

The built-in Wolfram Language function is \texttt{WhiteNoiseProcess}.
...
\includegraphics[scale=0.6,center]{images/white_noise_process.pdf}

\end{example}

\begin{exercise}
Study the Wiener process, which is given by
\begin{equation}
x_{n+1} = x_{n} + z_{n}
\end{equation}
with $z_{n}\sim \mathcal{N}\left(\mu,\sigma^2\right)$.
Use the function \texttt{WienerProcess} to generate paths starting at $x_{0} = 0$ and show that for non-zero $\mu$ the maximum of the distribution of endpoints after $n$ steps is shifted by a factor of $\mu$.
\end{exercise}

\begin{exercise}
Revisit the Random Walk process using the equation
\begin{equation}
x_{n+1} = x_{n} + z_{n}
\end{equation}
with $z_{n}\sim \alpha\mathcal{N}\left(+1,\sigma^2\right) + \left(1-\alpha\right)\mathcal{N}\left(-1,\sigma^2\right)$.
Show that the results from example \ref{ex:randomwalk} are reproduced in the limit $\sigma^2\rightarrow 0$.
\end{exercise}
...
\begin{definition}[Poisson Clock]
  ...
  \end{equation}
  $\nu$ is the clock rate (number of ticks in a certain time interval) and $t$ is the elapsed time. Because the expectation value is $\mu=\nu\ t$, the tick count is a measure of time.
\end{definition}
...
We want to create a parametric generic model of an atomic clock, which is able to generate data indistinguishable from measurement data of a passive hydrogen maser if the parameters are chosen properly. The treatment is based on the article \textbf{A mathematical model for the atomic clock error in case of jumps} \cite{zucca2015}. 

An atomic clock is a frequency source, which provides typically the following two outputs: a sinusoidal signal with a nominal frequency of $f_0$ and one pulse per second signal, which marks the start of second with its rising edge. The 1pps signal is generated from the high-frequency signal by counting the cycles and outputting a pulse when the cycle count reaches $f_0$. We focus on the high-frequency signal, which is characterized by its amplitude, the frequency deviation from the nominal frequency given by $y(t)=\frac{f(t)-f_0}{f_0}$ and phase time offset given by $x(t)=\frac{\phi(t)}{2\pi f_0}$. For simplicity we ignore amplitude variations and add the frequency drift or ageing $z(t)$ as a third parameter to describe the state of an atomic clock.  

The starting point for our analysis is a set of parametric equations, which describe the time evolution of the state $x(t)$, $y(t)$, $z(t)$ of an atomic clock including noise sources and artifacts like phase and frequency jumps. The parameters makes it possible to fit the model to actual phase data from measurements with a real clock.

  \begin{equation}
  \begin{aligned}
  \frac{dx(t)}{dt} = & y + w_{1} + a_{1}n_{1}\\
  \frac{dy(t)}{dt} = & z + w_{2} + a_{2}n_{2}\\
  \frac{dz(t)}{dt} = & w_{3} + a_{3}n_{3}\\  
  \end{aligned}
  \end{equation}
with $w_{i}\sim \mathcal{N}\left(\mu_{i},\sigma_{i}^2\right)$, 
$n_{i}\sim \mathcal{P}\left(\lambda_{i}\right)$ and $a_{i}\sim \mathcal{N}\left(0,\sigma_{i}^2\right)$.

One must consider that it is not possible to measure the phase of a single clock directly, because a phase comparison requires two inputs: the device under test and the reference clock. For simplicity, we assume that the contribution of the reference clock can be ignored, which is the good approximation for the phase data, we will use in this chapter and which comes from a frequency stability measurement between a passive hydrogen maser (the clock, which we are interested in) and an active hydrogen maser (the reference clock, which is more stable than its passive counter part). The measurements were conducted in a thermally-controlled environment, so that the effect of temperature-induced phase variations can be also neglected.  
...
We want to answer the questions how the spectral properties of the matrix like the distribution of eigenvalues in the limit $N\rightarrow\infty$ behaves. We have already seen an example for the case of a symmetric matrix $m$ with $m_{ij}\sim \mathcal{N}\left(0,1\right)$ and have found that the eigenvalues are distributed according to a SemiCircle pdf. We will now generalize the result by studying other distributions.
