...
Probabilistic models are described by a number of random variables $x_{1:n}$, a number of parameters $\theta_{1:m}$ and a joint probability density $p(x_{1:n}\,\vert\, \theta_{1:n})$, but there are also conditional dependencies between the random variables and parameters, which can expressed using a graph. Such a graph is also called a belief network \cite{barber2011} \cite{koller2009}.
...
The graph does exactly that: it specifies conditional independence relations, which can be used to simplify the joint probability density. This is basically an assumption or belief based on some prior knowledge. If this assumption is valid, has to be verified.\\
...
\end{equation}
There is no way for $x$ to influence $y$ if we fix $z$.  
\end{example}
