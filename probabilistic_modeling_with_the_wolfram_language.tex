...
\begin{example}[A Multi-modal Non-linear Time Series]
We will demonstrate with the following model \cite{klaas2012} how the particle filter works
\begin{equation}
\begin{aligned}
x_t&=\frac{x_{t-1}}{2}+\frac{25 x_{t-1}}{1+x_{t-1}^2}+\cos(1.2 t)+\eta_t\\
y_t&=\frac{x_{t}}{20}+\epsilon_t \\
\end{aligned}
\end{equation}
with the additive noise terms $\eta_t\sim \mathcal{N}(0,0.1)$ and $\epsilon_t
\sim \mathcal{N}(0,0.5)$. The corresponding distributions are given by
\begin{equation}
\begin{aligned}
p\left(x_{t+1}\,\vert\,x_{t}\right)&=\mathcal{N}\left(x_{t+1}-\frac{x_{t}}{2}-\frac{25 x_{t}}{1+x_{t}^2}-\cos(1.2 (t+1)), 0.1\right)\\
p\left(y_{t+1}\,\vert\, x_{t+1}\right)&=\mathcal{N}\left(y_{t+1} -\frac{x_{t+1}}{20} , 0.5\right) \\
\end{aligned}
\end{equation}
Because both distributions are Gaussian, importance sampling is not necessary here.
\end{example} 
