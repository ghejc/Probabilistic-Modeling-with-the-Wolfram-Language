...
\includegraphics[scale=0.9, center]{images/state_estimate_t1.pdf}

We repeat the steps for $t_2$ with the result

\includegraphics[scale=0.9, center]{images/state_estimate_t2.pdf}

and for $t_3$

\includegraphics[scale=0.9, center]{images/state_estimate_t3.pdf}
...
\begin{exercise}
At $t_4$ the result is that all system states have probability $0$, which cannot be the case, because the system is always in a particular state and as we see from the timeline graph, it is in state e. In this case a uniform probability distribution is returned, so we completely lost any information about the system state at $t_4$.

\includegraphics[scale=0.9, center]{images/state_estimate_t4.pdf}

Analyze this situation and come up with an explanation, why this happens. 
\end{exercise}
