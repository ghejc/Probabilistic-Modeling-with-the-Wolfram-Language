...
&=\frac{1}{Z}p(x_{t+1},y_{t+1},y_{t},\ldots)\\
&=\frac{1}{Z}p(y_{t+1}\,\vert\, x_{t+1})\int_{-\infty}^{+\infty}dx_{t} p(x_{t+1}\,\vert\, x_{t}) p(x_{t}\,\vert\, y_{t},y_{t-1},\ldots)
\end{aligned}
\end{equation}
and introduce the normalization factor $Z=p(y_{t+1},y_{t},\ldots)$.

\begin{definition}[Bayes Filter]
  \label{th:bayesfilter}
  \index{bayes filter}
  The probability density $p(x_{t+1}\,\vert\, y_{t+1},y_{t},\ldots)$ can be calculated iteratively from the last known $p(x_{t}\,\vert\,\ y_{t},y_{t},\ldots)$ by a prediction step 
\begin{equation}
...
\begin{equation}
p(x_{t+1}\,\vert\, y_{t+1},y_{t},\ldots)=\frac{1}{Z}p(y_{t+1}\,\vert\, x_{t+1})p(x_{t+1}\,\vert\, y_{t},\ldots)
\end{equation}
$p(x_{t+1}\,\vert\, x_{t})$ describes the time evolution of the system and $p(y_{t+1}\,\vert\, x_{t+1})$ the measurement process of the system state. If the system state can be controlled by an external input $u_t$, it is only necessary to modify $p(x_{t+1}\,\vert\, x_{t})$ by $p(x_{t+1}\,\vert\, x_{t}, u_{t})$. 
\end{definition} 
...
It turns out that the prior state estimate $p(x_0)$ before any observations are made can be chosen as a constant. After the first correction step when the normalization is performed the constant will anyhow be cancelled out.\\  

The Bayes filter framework is the conceptual basis of a large number of existing models:
\begin{itemize}
\item
Kalman filters
\item
