...
The system state at time $t$ is $(\vec{r},\vec{v})$, where $\vec{r}$ is the coordinate and $\vec{v}$ is the velocity of the ball. The origin of the coordinate system is the center of the box. The system boundaries are at $x=\pm 1$ and $y=\pm 1$.\\

We implement a function \texttt{Trajectory}, which returns the trajectory of the ball.
\begin{mathematica}
Trajectory[initialPosition_, initialSpeed_, initialTime_, size_,
...
\begin{mathematica}
trajectory = Trajectory[{0.1, 0.4}, {0.1, 0.2}, 0, 10, {1, -1, 1, -1}];
...
\includegraphics[scale=0.6, center]{images/ball_trajectory.pdf}
