...
The function \texttt{sampleFromHistogram} has three arguments: the bin interval limits \texttt{bins} as a list of $N+1$ elements, the normalized bin heights \texttt{binHeights} as a list with $N$ elements and the number of samples \texttt{numSamples}. It calculates first the center coordinates and widths of the bins and then the cumulative sum of the bin heights, where zero is appended at the beginning of the list. \texttt{numSamples} random numbers are generated to sample a list of bin indices. This is done by subtracting the random number from each element of the cumulative sum and keep only the largest element of the non-positive results. The index is then used to generate uniformly distributed random numbers inside the corresponding bin.
\begin{mathematica}
sampleFromHistogram[bins_, binHeights_, numSamples_Integer] := Module[
  {binWidths, binCenters, cumSum, randomNumbers, binIndices, histSamples},
  binWidths = Differences[bins];
  ...
  binIndices = Flatten[Map[Ordering[Select[cumSum - #, NonPositive], -1] &,
                           randomNumbers]];
  histSamples = Map[RandomReal[{binCenters[[#]] - binWidths[[#]]/2,
                                binCenters[[#]] + binWidths[[#]]/2}] &, binIndices];
  Return[histSamples]]
