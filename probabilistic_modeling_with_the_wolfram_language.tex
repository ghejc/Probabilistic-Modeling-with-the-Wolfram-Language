...
\begin{exercise}
 Show that the mean and variance of a random variable $x$ distributed as a Poisson distribution $\mathcal{P}\left(x\,\vert\,\lambda\right)$ is given by $\mu=\lambda$ and $\sigma^2=\lambda$ if the possible values $x$ can take are $x_k=k$. 
\end{exercise}
