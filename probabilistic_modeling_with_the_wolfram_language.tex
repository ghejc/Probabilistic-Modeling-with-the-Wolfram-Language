...
The Wolfram language \cite{wolfram2015, dorn2018} has a clear and consistent design with a syntax close to the mathematical notation and has all the built-in knowledge to perform symbolic calculations where results in a closed-form are available and numerical evaluations in the case where this is not possible. Especially the Probability and Statistics part, which is used heavily in this book, has been improved significantly in the newer version of the language and the available functions can be easily applied to all kind of statistical problems. The visualization capabilities of the Wolfram language are outstanding and every figure in this book was generated with the version integrated into Mathematica 12.0 \cite{mathematica12}.\\
...
\end{mathematica}
The result is a \texttt{ConditionalExpression} being $1$ under the constraints $\rho < 1$ and $\sigma_1 > 0$. These conditions can be added as \texttt{Assumptions} to \texttt{ProbabilityDistribution}.
