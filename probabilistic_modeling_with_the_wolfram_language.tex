...
An atomic clock is a frequency source, which provides typically the following two outputs: a sinusoidal signal with a nominal frequency of $f_0$ and one pulse per second signal, which marks the start of second with its rising edge. The 1pps signal is generated from the high-frequency signal by counting the cycles and outputting a pulse when the cycle count reaches $f_0$.  

The starting point for our analysis are a set of parametric equations, which describe the phase of an atomic clock over time including noise sources and artifacts like phase and frequency jumps. The parameters makes it possible to fit the model to actual phase data from measurements with a real clock.
