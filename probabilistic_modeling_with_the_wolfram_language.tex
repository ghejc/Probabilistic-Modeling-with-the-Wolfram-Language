...
\end{mathematica}
 ...
 \includegraphics[scale=0.9]{images/gaussian_mixture_model_sampling.pdf}
\end{example}
...
Probabilistic models are described by a number of random variables $x_{1:n}$, a number of parameters $\theta_{1:m}$ and a joint probability density $p(x_{1:n}\,\vert\, \theta_{1:n})$, but there are also conditional dependencies between the random variables and parameters, which can expressed using a graph.\\

The following Wolfram Language code draws a graph with 3 random variables $x$, $y$, $z$ and one parameter $\theta$. The function \texttt{Graph} gets an adjacency list as the first argument, which contains a list of edges. The nodes or vertices are numbered from $1$ to $4$ and \texttt{i \textbackslash\lbrack DirectedEdge\rbrack \,j} means that there is a directed edge from node $i$ to $j$. This expresses a conditional dependence of node $j$ on node $i$.\\
...
\end{mathematica}
 \includegraphics[scale=0.9]{images/simple_graph.pdf}
